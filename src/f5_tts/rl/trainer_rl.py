...
class GRPOTrainer:
    def __init__(
        ...
        last_per_steps=None,
        accelerate_kwargs: Optional[Dict] = None,
        ema_kwargs: Optional[Dict] = None,
    ):
        ...
        ref_model_cfg = dict(dim=1024, depth=22, heads=16, ff_mult=2, text_dim=512, conv_layers=4)
        ema_model = load_model("F5-TTS", "F5TTS_ref", DiT, ref_model_cfg, "last")
    ...
    def _repeat_batches(self, batches):
        repeated = []
        ...
        for epoch in range(skipped_epoch, self.epochs):
            ...
            for batch in progress_bar:
                with self.accelerator.accumulate(self.model):
                    ...
                    prompt_idx = prompt_idx.unsqueeze(-1).repeat(1, 1, 100)
                    ...
                    # cross-device reduce rewards; avoid illegal filenames on Windows
                    if torch.cuda.is_available() and torch.cuda.device_count() > 0:
                        rewards_list = rewards.view(-1).tolist()
                        rewards_list = [str(item) for item in rewards_list]
                        safe_name = f"cuda-{self.accelerator.process_index}.tmp"
                        ...
                        for i in range(torch.cuda.device_count()):
                            temp = []
                            with open(f"cuda-{i}.tmp", encoding="utf-8") as f:
                        ...
                        raise RuntimeError("CUDA is required for RL reward aggregation; no CPU fallback implemented.")
