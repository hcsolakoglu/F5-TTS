...
def run_correctness_probe(repo: Path, data_file: Path, out_dir: Path, candidate_names: list[str], args: argparse.Namespace) -> None:
    ...
    probe.write_text(
        textwrap.dedent(
            f"""
            import json, os, sys, copy, torch
            from pathlib import Path
            repo=Path({str(repo)!r}); data_file=Path({str(data_file)!r}); out_dir=Path({str(out_dir)!r})
            sys.path.insert(0, str(repo/'src'))
            ...
            torch.manual_seed({args.seed})
            if torch.cuda.is_available():
                torch.cuda.manual_seed_all({args.seed}); torch.backends.cuda.matmul.allow_tf32=False; torch.backends.cudnn.allow_tf32=False; torch.set_float32_matmul_precision('highest')
            obj=torch.load(data_file, map_location='cpu')
            batch=[{{'mel_spec': obj['mels'][i], 'text': obj['texts'][i]}} for i in range(min(4, len(obj['mels'])))]
            b=collate_fn(batch)
            mel=b['mel'].permute(0,2,1).cuda(); lens=b['mel_lengths'].cuda(); text=b['text']
            vocab_char_map, vocab_size=get_tokenizer('', 'byte')
            model_cfg=dict(dim={args.dim}, depth={args.depth}, heads={args.heads}, ff_mult=2, text_dim={args.text_dim}, text_mask_padding=True, qk_norm=None, conv_layers=4, pe_attn_head=None, attn_backend='torch', attn_mask_enabled=False, checkpoint_activations=False)
            ...
            def build():
                ...
                return CFM(transformer=DiT(**model_cfg, text_num_embeds=vocab_size, mel_dim=100), mel_spec_kwargs=mel_spec_kwargs, vocab_char_map=vocab_char_map).cuda().train()
            base=build(); cand_template=copy.deepcopy(base)
            torch.manual_seed(999); torch.cuda.manual_seed_all(999)
            prepared=base._prepare_training_inputs(mel.clone(), text, lens.clone())
            loss, cond, pred=base._run_loss_core(*prepared); loss.backward()
            ...
            for name,cfg in candidates.items():
                if name not in {candidate_names!r}: continue
                if not cfg.get('compile'): continue
                m=copy.deepcopy(cand_template).cuda().train()
                kwargs={{'backend':'inductor','fullgraph':False,'dynamic':None,'runtime_fallback':False}}
                if cfg.get('options') is not None: kwargs['options']=cfg['options']
                elif cfg.get('mode') is not None: kwargs['mode']=cfg['mode']
                m.compile_training_core(**kwargs)
                if cfg.get('mark_step'):
                    torch.compiler.cudagraph_mark_step_begin()
                args2=tuple(x.detach().clone() if torch.is_tensor(x) else x for x in prepared)
                closs,ccond,cpred=m._run_loss_core(*args2); closs.backward()
                max_grad=0.0
                ...
                results.append({{'candidate':name,'loss_abs_diff':float(abs(loss.detach().cpu()-closs.detach().cpu())),'pred_max_abs_diff':float((pred.detach().cpu()-cpred.detach().cpu()).abs().max()),'grad_max_abs_diff':max_grad,'state':m.training_compile_state}})
            (out_dir/'correctness.json').write_text(json.dumps(results, indent=2, sort_keys=True)+'\n')
            print('CORRECTNESS_JSON='+json.dumps(results, sort_keys=True))
            """
        ),
        encoding="utf-8",
    )
...
def orchestrate(args: argparse.Namespace) -> None:
    ...
    for name in selected:
        ...
        env["TORCH_LOGS"] = env.get("TORCH_LOGS", "")
        env["WANDB_DISABLED"] = "true"
        print(f"\n=== candidate {name} ===", flush=True)
        start = time.perf_counter()
        try:
            subprocess.run(common + ["--candidate", name], env=env, check=True)
        finally:
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--candidates",
        default="eager,compiled_default,reduce_overhead_markstep,combo_cudagraphs_markstep",
    )
